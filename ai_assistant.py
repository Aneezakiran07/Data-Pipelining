...
GUIDE_ITEMS = [
    ...
    {
        "tab": "Filter and Inspect",
        "title": "Inspect rows before cleaning",
        "desc": "The Filter and Inspect tab lets you slice your data by any column and condition without modifying anything. Use it to spot dirty values before deciding which operation to run. You can stack multiple filters and export matching rows as CSV or Excel.",
        "tips": [
            "Filters never modify your data — completely safe to explore.",
            "Switch between ALL (AND) and ANY (OR) mode to change how filters combine.",
            "Export filtered rows to investigate suspicious data outside the app.",
        ],
    },
    {
        "tab": "Recommendations",
    ...
    },
    {
        ...
        "desc": "The Clean tab has Basic Cleaning buttons (Strip Whitespace, Drop Duplicate Rows, Drop Duplicate Cols, Clean String Edges), Advanced Cleaning (Smart Column Cleaner, Handle Missing Values), and tools for Find and Replace, Column Type Override, Split Column, Merge Columns, Rename Columns, and Data Type Guesser.",
        "tips": [
...
def _render_ai_analysis_section(cdf, file_id):
    ...
    st.caption("AI reads your specific dataset and tells you exactly what to fix and how.")
    ...
    st.caption(
        "Each card tells you the problem, which tab to go to, what to click, "
        "and a shortcut using the AI Cleaner."
    )
    ...
    for fix in fixes:
        ...
        col_label = (
            f"Column: **{column}**" if column and column != "ALL"
            else "Affects the **whole dataset**"
        )
